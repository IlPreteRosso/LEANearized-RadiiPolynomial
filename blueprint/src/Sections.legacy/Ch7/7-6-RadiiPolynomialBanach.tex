...
\end{equation*}
  ...
  \begin{equation*}
  \lVert \text{DT}(c)\rVert \leq Z_0 + Z_1 + Z_2(r) \cdot r = Z(r).
\end{equation*}
\end{lemma}
...
\end{equation*}
  ...
  \eqref{eq:7.33}:
  \begin{equation*}
  \lVert \text{DT}(c)\rVert \leq Z_0 + Z_1 + Z_2(r) \cdot r.
\end{equation*}
\end{proof}
...
\begin{lemma}[Simple derivative bound]\label{lem:newton_deriv_bound_simple}
  ...
  \begin{equation*}
  \lVert I_E - A \circ \text{Df}(\bar{x})\rVert \leq Z_0
\end{equation*}
\begin{equation*}
  \lVert A \circ (\text{Df}(c) - \text{Df}(\bar{x}))\rVert \leq Z_2(r) \cdot r
\end{equation*}
  imply $\lVert \text{DT}(c)\rVert \leq Z_0 + Z_2(r) \cdot r$.
\end{lemma}
...
\begin{lemma}[Ball mapping property]\label{lem:maps_ball_to_itself}
  ...
  Given $T: E \to E$ differentiable with:
    (8) $\lVert T(\bar{x}) - \bar{x}\rVert \leq Y_0$
    (9) $\lVert \text{DT}(c)\rVert \leq Z(r_0)$ for all $c \in \overline{B}_{r_0}(\bar{x})$
    (10) $p(r_0) = (Z(r_0) - 1)r_0 + Y_0 < 0$
...
\end{equation*}
  ...
  \begin{equation*}
  \lVert T(x) - T(\bar{x})\rVert \leq Z(r_0) \cdot \lVert x - \bar{x}\rVert \leq Z(r_0) \cdot r_0.
\end{equation*}
...
\begin{proof}
  ...
  \textbf{Step 2: Verify conditions of Theorem~\ref{thm:general_fixed_point}.}
    (11) By Lemma~\ref{lem:newton_Y_bound}: $\lVert T(\bar{x}) - \bar{x}\rVert \leq Y_0$
    (12) By Lemma~\ref{lem:newton_deriv_bound_general}:
          $\lVert \text{DT}(c)\rVert \leq Z(r_0)$ for $c \in \overline{B}_{r_0}(\bar{x})$
    (13) The polynomial condition:
...
\begin{proof}
  ...
  $Z(r_0) < 1$, we have $\lVert I_E - A \circ \text{Df}(\tilde{x})\rVert < 1$.
