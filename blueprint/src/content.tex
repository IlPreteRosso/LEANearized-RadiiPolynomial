...
%----------------------------------------------------
\begin{dfn}[Finite-dimensional data]\label{def:data}
  \lean{RP.Data}
  Fix a norm $\|\cdot\|$ on $\Bbb R^n$ (or $\Bbb C^n$). Let $f:\Bbb R^n\to\Bbb R^n$ be $C^1$,
  $\bar x\in\Bbb R^n$ a numerical approximation to a zero of $f$, and
  $A\in M_n(\Bbb R)$ (typically $A\approx Df(\bar x)^{-1}$).
\end{dfn}

%----------------------------------------------------
\begin{dfn}[Newton-like map]\label{def:T}
  \lean{RP.T}
  \uses{def:data}
  Define $T:\Bbb R^n\to\Bbb R^n$ by $T(x)=x-A f(x)$.
\end{dfn}

%----------------------------------------------------
\begin{dfn}[A priori bounds $Y_0,Z_0,Z_2$]\label{def:bounds}
  \lean{RP.Bounds}
  \uses{def:data,def:T}
...
%----------------------------------------------------
\begin{dfn}[Radii polynomial]\label{def:rpoly}
...
%----------------------------------------------------
\begin{lem}[Derivative bound]\label{lem:DT-bound}
  \lean{RP.DT_bound}
...
%----------------------------------------------------
\begin{lem}[Ball invariance and contraction criterion]\label{lem:fixed-point-radii}
...
%----------------------------------------------------
\begin{thm}[Radii polynomials in finite dimensions]\label{thm:radii-poly-fd}
