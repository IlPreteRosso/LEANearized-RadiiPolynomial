...
\subsection{Radii polynomials in finite dimensions}\label{sec:radii-poly-fd}
...
% \begin{theorem}[Contraction Mapping Theorem]\label{thm:CM_existsUnique}
...
\begin{lemma}[Fixed-point radii]\label{lem:FixedPointRadii}
  ...
  \uses{lem:MVI,lem:DTbound,dfn:Ball,thm:CM_existsUnique}
...
\begin{theorem}[Radii polynomials in finite dimensions]\label{thm:RadiiPolyFD}
  ...
  \uses{dfn:RPoly,lem:DTbound,lem:FixedPointRadii,lem:Neumann,thm:CM_existsUnique}
...
\begin{proof}
  ...
  Apply Lemma~\ref{lem:FixedPointRadii} with $r=r_0$ to obtain (i) and (ii); the existence and uniqueness follow from Theorem~\ref{thm:CM_existsUnique}.
