...
\section{Weighted Sequence Spaces (Section 7.4)}\label{sec:weighted_l1}
...
\subsection{Scaled Real Numbers}\label{subsec:scaled_real}
...
\subsection{Weighted $\ell^p$ Spaces}\label{subsec:weighted_lp}
...
They are essential for the Fréchet derivative formula in Section~\ref{sec:example_7_7}.
...
is compatible with ring multiplication. This is essential for the Fréchet derivative formula
...
\begin{corollary}[Commutative Banach algebra]\label{cor:l1Weighted_Banach_algebra}
  \uses{lem:lpWeighted_complete,thm:l1Weighted_CommRing,thm:l1Weighted_NormedRing,thm:l1Weighted_NormOneClass,thm:l1Weighted_SMulCommClass,thm:l1Weighted_IsScalarTower}
  ...
  \begin{enumerate}
    ...
    \item $\|1\| = 1$
    \item Scalar-ring compatibility ($\mathbb{R}$-algebra structure)
  \end{enumerate}
\end{corollary}
...
The formalization separates concerns into two files:
