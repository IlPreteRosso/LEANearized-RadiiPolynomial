...
\begin{theorem}[Existence of fixed point]\label{thm:example_7_7_existence}
  ...
  \leanok
  \uses{thm:simple_radii_poly_same,lem:radii_poly_neg,def:F_map,def:concrete_approx,cor:l1Weighted_Banach_algebra}
...
\begin{proof}
  ...
  The Banach algebra structure of $\ell^1_\nu$ (Corollary~\ref{cor:l1Weighted_Banach_algebra})
  provides the necessary completeness and submultiplicativity.
\end{proof}
...
\begin{theorem}[Cauchy product is formal multiplication]\label{thm:coeff_mul_eq_cauchy}
  ...
  \leanok
  \uses{def:to_power_series,def:CauchyProduct,thm:toPowerSeries_mul}
  For $a, b \in \ell^1_\nu$:
  \begin{equation*}
...
\begin{lemma}[Absolute convergence]\label{lem:summable_eval}
  ...
  \leanok
  \uses{def:l1Weighted,def:eval,lem:l1_mem_iff}
...
\begin{theorem}[Mertens' theorem: evaluation commutes with multiplication]\label{thm:eval_mul}
  ...
  \leanok
  \uses{def:CauchyProduct,lem:summable_eval,thm:norm_mul_le}
  For $a, b \in \ell^1_\nu$ and $|z| \leq \nu$:
  \begin{equation*}
