...
\begin{theorem}[Fixed point theorem with radii polynomial]\label{thm:fixed_point_radii}
  ...
  \begin{align}
    \lVert T(\bar{x}) - \bar{x}\rVert &\leq Y_0 \label{eq:2.10}\\
...
\begin{theorem}[Radii polynomial theorem in finite dimensions]\label{thm:radii_poly_fd}
  ...
  \begin{align}
    \lVert Af(\bar{x})\rVert &\leq Y_0 \label{eq:2.14}\\
    \lVert I - A\text{Df}(\bar{x})\rVert &\leq Z_0 \label{eq:2.15}\\
...
\begin{proof}
  ...
  We have $\lVert T(\bar{x}) - \bar{x}\rVert = \lVert Af(\bar{x})\rVert \leq Y_0$ by \eqref{eq:2.14}.
  
  For $c \in \overline{B}_{r_0}(\bar{x})$,
  \begin{align*}
    \lVert \text{DT}(c)\rVert &= \lVert I - A\text{Df}(c)\rVert\\
    &\leq \lVert I - A\text{Df}(\bar{x})\rVert + \lVert A[\text{Df}(\bar{x}) - \text{Df}(c)]\rVert\\
  ...
  In particular, $\lVert I - A\text{Df}(\bar{x})\rVert \leq Z_0 \leq Z(r_0) < 1$, so 
  ...
  $\lVert \text{DT}(\tilde{x})\rVert = \lVert I - A\text{Df}(\tilde{x})\rVert \leq Z(r_0) < 1$. 
