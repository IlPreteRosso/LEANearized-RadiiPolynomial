...
\begin{definition}[Operator norm]\label{def:operator_norm}
  ...
  $A: X \to Y$ be a continuous linear map. The \textit{operator norm} on $A$ is given by
  \begin{equation*}
...
\begin{proposition}[Properties of operator norm]\label{prop:operator_norm_properties}
  % Mathlib: ContinuousLinearMap.opNorm_comp_le, ContinuousLinearMap.le_opNorm
...
\begin{theorem}[Mean Value Theorem]\label{thm:mean_value}
  ...
  Consider $x, y \in U$ such that the line segment $(1-t)x + ty$, $t \in [0,1]$,
...
\begin{proof}
  For each $i \in \{1,\ldots,n\}$, define $g_i: [0,1] \to \mathbb{R}$ by $g_i(t) = f_i((1-t)x + ty)$.
  By the fundamental theorem of calculus and chain rule:
  \begin{equation*}
  f_i(y) - f_i(x) = g_i(1) - g_i(0) = \int_0^1 g'_i(t) \, dt = \int_0^1 \text{D}f_i((1-t)x + ty) \cdot (y-x) \, dt.
  \end{equation*}
  Collecting all components yields the result.
\end{proof}

\begin{corollary}[Mean Value Inequality]\label{cor:mean_value_inequality}
  % Mathlib: Convex.norm_image_sub_le_of_norm_fderiv_le
  \uses{thm:mean_value,def:operator_norm,prop:operator_norm_properties}
  ...
  \begin{equation*}
  \lVert f(y) - f(x)\rVert \leq \left(\sup_{z \in \overline{B}_\rho(x_0)} 
    \lVert \text{Df}(z)\rVert\right) \lVert y-x\rVert,
  \end{equation*}
  where $\lVert \text{Df}(\cdot)\rVert$ denotes the operator norm.
\end{corollary}

\begin{proof}
  The line segment from $x$ to $y$ lies in $\overline{B}_\rho(x_0)$ by convexity.
  By the Mean Value Theorem:
  \begin{equation*}
  \lVert f(y) - f(x)\rVert = \left\lVert \int_0^1 \text{Df}((1-t)x + ty)(y-x) \, dt \right\rVert
  \leq \int_0^1 \lVert \text{Df}((1-t)x + ty)\rVert \lVert y-x\rVert \, dt.
  \end{equation*}
  Since $(1-t)x + ty \in \overline{B}_\rho(x_0)$ for all $t \in [0,1]$, we have
  $\lVert \text{Df}((1-t)x + ty)\rVert \leq \sup_{z \in \overline{B}_\rho(x_0)} \lVert \text{Df}(z)\rVert$,
  yielding the result.
\end{proof}
