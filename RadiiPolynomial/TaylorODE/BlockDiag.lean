...
lemma tailDiag_norm_bound {ν : PosReal} {N : ℕ} (A : BlockDiagOp ν N) (a : l1Weighted ν) :
  ...
  have h_pw : ∀ n : {n : ℕ // N < n},
    ...
    exact mul_le_mul_of_nonneg_right (A.tailBound_spec n hn)
      (mul_nonneg (abs_nonneg _) (pow_nonneg (PosReal.coe_nonneg ν) n))
  ...
  exact (h_sum_rhs.of_nonneg_of_le
    (fun _ => mul_nonneg (abs_nonneg _) (pow_nonneg (PosReal.coe_nonneg ν) _)) h_pw).tsum_le_tsum
    h_pw h_sum_rhs
...
lemma BlockDiagOp.action_mem {ν : PosReal} {N : ℕ} (A : BlockDiagOp ν N) (a : l1Weighted ν)
  ...
  refine Summable.of_nonneg_of_le
    (fun n => mul_nonneg (abs_nonneg _) (pow_nonneg (PosReal.coe_nonneg ν) n))
    (f := g) (fun n => ?_) ?_
  · -- Pointwise bound
    ...
    · -- n > N: |tailDiag n * a_n| * ν^n ≤ C * |a_n| * ν^n
      ...
      rw [abs_mul, mul_assoc]
      exact (mul_le_mul_of_nonneg_right (A.tailBound_spec n hn)
        (mul_nonneg (abs_nonneg _) (pow_nonneg (PosReal.coe_nonneg ν) n))).trans
