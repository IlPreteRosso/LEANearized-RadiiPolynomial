...
theorem invAtPrec_mul_le_one {x : Dyadic} (hx : 0 < x) (prec : Int) :
  ...
  | ofOdd n k hn =>
    simp only
    have h_le : ((ofOdd n k hn).toRat.inv.toDyadic prec).toRat ≤ (ofOdd n k hn).toRat.inv :=
      Rat.toRat_toDyadic_le
...
theorem one_lt_invAtPrec_add_inc_mul {x : Dyadic} (hx : 0 < x) (prec : Int) :
  ...
  | ofOdd n k hn =>
    simp only
    have h_le : (ofOdd n k hn).toRat.inv <
    ...
    calc
      1 = (ofOdd n k hn).toRat.inv * (ofOdd n k hn).toRat := by
        symm; apply Rat.inv_mul_cancel
        exact Rat.ne_of_gt h_pos
      _ < ((ofOdd n k hn).toRat.inv.toDyadic prec + ofIntWithPrec 1 prec).toRat *
          (ofOdd n k hn).toRat := Rat.mul_lt_mul_of_pos_right h_le h_pos
