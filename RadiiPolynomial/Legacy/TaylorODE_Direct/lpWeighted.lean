import RadiiPolynomial.TaylorODE_Direct.ScaledReal
import RadiiPolynomial.TaylorODE_Direct.CauchyProduct
...
- `l1Weighted.matrixColNorm`, `l1Weighted.finWeightedMatrixNorm`: finite weighted matrix norms
- `l1Weighted.arrayColNormIccSum`: array-backed finite column sum used by LeanCert bounds
...
### Finite-Norm Bridge API (for direct certified bounds)
- `l1Weighted.norm_eq_Icc_sum_of_support`: rewrite `‖a‖` as finite `Icc` sum under support cutoff
- `l1Weighted.finWeightedMatrixNorm_le_of_matrixColNorm_le`: reduce matrix norm goals to per-column goals
- `l1Weighted.matrixColNorm_eq_arrayColNormIccSum`:
  identify abstract `matrixColNorm` with computable array formula
- `l1Weighted.matrixColNorm_le_of_arrayColNormIccSum`:
  transfer array bounds into `matrixColNorm` bounds
...
/-- For finitely-supported `a` (support ≤ `M`), rewrite the norm as a finite `Icc` sum. -/
lemma norm_eq_Icc_sum_of_support (a : l1Weighted ν) (M : ℕ)
  ...
  exact tsum_eq_sum fun n hn => by
    ...
    simp [lpWeighted.toSeq] at hzero
    simp [hzero]
...
/-! ### Weighted Term Nonnegativity -/

/-- The weighted term `|a| * ν^n` is nonneg. This appears ubiquitously in ℓ¹_ν proofs. -/
lemma weighted_term_nonneg (a : ℝ) (n : ℕ) : 0 ≤ |a| * (ν : ℝ) ^ n :=
  mul_nonneg (abs_nonneg _) (pow_nonneg ν.coe_nonneg _)

/-! ### Matrix Column Norm: Simp Lemma -/

/-- Key identity: `matrixColNorm j * ν^j = ∑_i |A i j| * ν^i`.
    This eliminates the `field_simp` friction when working with `matrixColNorm`. -/
@[simp]
lemma matrixColNorm_mul_pow (j : Fin (N + 1)) :
    matrixColNorm ν A j * (ν : ℝ) ^ (j : ℕ) = ∑ i : Fin (N + 1), |A i j| * (ν : ℝ) ^ (i : ℕ) := by
  rw [matrixColNorm]; field_simp [pow_ne_zero _ (PosReal.coe_ne_zero ν)]
...
lemma matrixColNorm_nonneg (A : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ) (j : Fin (N + 1)) :
  ...
  apply mul_nonneg
  · apply div_nonneg zero_le_one (pow_nonneg ν.coe_nonneg _)
  · exact Finset.sum_nonneg fun i _ => weighted_term_nonneg _ _
...
/-- Bound the weighted matrix norm by a uniform bound on all column norms. -/
...
/-- `matrixColNorm` with denominator distributed termwise across the finite sum. -/
lemma matrixColNorm_eq_sum_div (A : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ) (j : Fin (N + 1)) :
    matrixColNorm ν A j =
      ∑ i : Fin (N + 1), |A i j| * (ν : ℝ) ^ (i : ℕ) / (ν : ℝ) ^ (j : ℕ) := by
  rw [matrixColNorm_eq, Finset.sum_div]

/-- Array-backed column expression for weighted matrix column norms over `Icc 0 N`.

    This keeps explicit finite-sum formulas in the norm file, while bound files can
    refer to a named object. -/
noncomputable def arrayColNormIccSum (ν : PosReal) (N : ℕ) (col : Array ℚ) (j : ℕ) : ℝ :=
  ∑ k ∈ Finset.Icc (0 : ℕ) N,
    |(col.getD k 0 : ℝ)| * (ν : ℝ) ^ k / (ν : ℝ) ^ j

/-- Identify `matrixColNorm` with an array-backed `Icc` sum over `0..N`. -/
lemma matrixColNorm_eq_arrayColNormIccSum
    (ν : PosReal) (N : ℕ)
    (M : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ) (col : Array ℚ) (j : Fin (N + 1))
...
/-- Transfer a computable array-column bound to a `matrixColNorm` bound. -/
lemma matrixColNorm_le_of_arrayColNormIccSum
    (ν : PosReal) (N : ℕ)
    (M : Matrix (Fin (N + 1)) (Fin (N + 1)) ℝ) (col : Array ℚ) (j : Fin (N + 1)) (C : ℝ)
    (hM : ∀ i : Fin (N + 1), M i j = ((col).getD (i : ℕ) 0 : ℝ))
