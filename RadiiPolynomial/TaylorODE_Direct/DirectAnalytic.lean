...
* `DirectCore.analyticSolution_eq_sqrt`: Branch selection via IVT shows x̃(λ - λ₀) = √λ
...
This completes the formalization of Direct Taylor ODE from the radii polynomial textbook.
-/
...
def paramPowerSeries (lam0 : ℝ) : PowerSeries ℝ :=
  PowerSeries.mk (DirectCore.paramSeq lam0)

@[simp]
theorem coeff_paramPowerSeries (lam0 : ℝ) (n : ℕ) :
    coeff n (paramPowerSeries lam0) = DirectCore.paramSeq lam0 n :=
  PowerSeries.coeff_mk n _
...
theorem paramPowerSeries_eq (lam0 : ℝ) :
  ...
  match n with
  | 0 => simp [DirectCore.paramSeq]
  | 1 => simp [DirectCore.paramSeq]
  | n + 2 => simp [DirectCore.paramSeq]
...
theorem toPowerSeries_sq_eq_param (a : l1Weighted ν) (lam0 : ℝ)
    (hF : ∀ n, (lpWeighted.toSeq a ⋆ lpWeighted.toSeq a) n = DirectCore.paramSeq lam0 n) :
...
theorem eval_paramSeq (lam0 z : ℝ) :
    ∑' n, DirectCore.paramSeq lam0 n * z ^ n = lam0 + z := by
  have h : ∀ n ≥ 2, DirectCore.paramSeq lam0 n * z ^ n = 0 := by
    intro n hn
    simp only [DirectCore.paramSeq]
    match n with
    | 0 => omega
    | 1 => omega
    | n + 2 => simp
  rw [tsum_eq_sum (s := {0, 1})]
  · simp [DirectCore.paramSeq, pow_zero, pow_one]
  · intro n hn
...
/-! ## Connection to Direct Taylor ODE -/
...
theorem analyticSolution_is_sqrt {ν : PosReal} (aTilde : l1Weighted ν) (lam0 : ℝ)
    (hF : ∀ n, (lpWeighted.toSeq aTilde ⋆ lpWeighted.toSeq aTilde) n = DirectCore.paramSeq lam0 n)
...
theorem analyticSolution_eq_sqrt {ν : PosReal} (aTilde : l1Weighted ν) (lam0 : ℝ)
  ...
  have hpos : 0 < analyticSolution aTilde (lam - lam0) := by
    ...
    have h_cont : ContinuousOn (fun z => DirectCore.analyticSolution aTilde z) (Set.Icc (-ν) (ν)) := by
    ...
    have h_ivt : ∃ z ∈ Set.Icc (lam - lam0) 0, DirectCore.analyticSolution aTilde z = 0 := by
      apply_rules [intermediate_value_Icc];
      · -- Since $lam - lam0 \leq 0$, we have $lam - lam0 \leq 0$.
        apply le_of_not_gt; intro h_neg;
        have h_ivt : ∃ z ∈ Set.Ioo 0 (lam - lam0), DirectCore.analyticSolution aTilde z = 0 := by
        ...
        have h_ivt : DirectCore.analyticSolution aTilde z ^ 2 = lam0 + z := by
          apply eval_sq_eq;
