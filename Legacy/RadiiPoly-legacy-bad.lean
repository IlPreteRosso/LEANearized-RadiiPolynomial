...
/-- Uniqueness of fixed points for contractions -/
lemma unique_fixedPoint_of_contracting
  {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
  {T : E → E} {κ : ℝ} {x y : E}
  -- (hκ : 0 ≤ κ)
  (hκ1 : κ < 1)
  (h_contract : dist (T x) (T y) ≤ κ * dist x y)
  (hx : T x = x) (hy : T y = y) :
  x = y := by
  -- If T x = x and T y = y, then dist x y = dist (T x) (T y) ≤ K * dist x y
  by_contra h_ne
  have h_dist_pos : 0 < dist x y := dist_pos.mpr h_ne
  -- From fixed points: dist x y = dist (T x) (T y)
  have h_eq : dist x y = dist (T x) (T y) := by
    rw [hx, hy]
  -- Apply contraction: dist (T x) (T y) ≤ K * dist x y
  -- So dist x y ≤ K * dist x y
  have h_le : dist x y ≤ κ * dist x y := by
    calc dist x y
        = dist (T x) (T y) := h_eq
      _ ≤ κ * dist x y := h_contract
  -- Therefore (1 - κ) * dist x y ≤ 0
  have h1 : (1 - κ) * dist x y ≤ 0 := by linarith
  -- Since 1 - κ > 0 and dist x y > 0, this is a contradiction
  have h_pos : 0 < 1 - κ := by linarith
  have h_prod_pos : 0 < (1 - κ) * dist x y := mul_pos h_pos h_dist_pos
  linarith
